...
public class SRWebClient : NSObject
{
    ...
    */
    ...
    public class func POST2(url: String, success:SuccessHandler?, failure:FailureHandler?) -> SRWebClient {
        return SRWebClient.POST(url).send(success, failure: failure)
    }
    
    /*public class func POST3(url: String, data:RequestData?, headers:Headers?) -> SRWebClient {
    return SRWebClient.POST(url).headers(headers).data(data)
    }
    
    public class func POST4(url: String, data:RequestData?, success:SuccessHandler?, failure:FailureHandler?) -> SRWebClient {
    return SRWebClient.POST(url, data: data, headers:nil).send(success, failure)
    }
    
    public class func POST5(url: String, data:RequestData?, headers:Headers?, success:SuccessHandler?, failure:FailureHandler?) -> SRWebClient {
    ...
    *  @return self instance to support function chaining
    */
    public func datar(image:NSData, fieldName:String, data:RequestData?) -> SRWebClient {
    ...
    */
    public func send(success:SuccessHandler?, failure:FailureHandler?) -> SRWebClient {
        var blockOperation:NSBlockOperation = NSBlockOperation(block: {() -> Void in
            ...
            //let result:NSData? = NSURLConnection.sendAsynchronousRequest(self.urlRequest!, returningResponse: &response)
